...
short_summary: "How to retrieve and inject agents/ context for OpenAI models using ExoFrame embeddings and chunk tooling."
version: "0.1"
...
Key points
- Use RAG by default: retrieve agents/ context before proposing code changes.
...
ExoFrame precomputes embeddings for all agent documentation in `agents/` and splits docs into `agents/chunks/*.txt` for retrieval. This guide shows a predictable, tool-centric workflow to provide OpenAI models (gpt-4o family) the most relevant ExoFrame context with minimal prompt bloat.
...
"Before answering, retrieve agents/ context using inject_agent_context. Use 2–3 chunks for simple tasks, 4–6 for standard, 8–10 for complex. Cite the agent docs used by path. Provide diffs first, then verification steps."
...
- Precomputed embeddings: import JSON vectors under `agents/embeddings/` then rebuild.

## Freshness contract (don’t skip)

If you add or update anything under `agents/`, do this before trusting RAG output:
