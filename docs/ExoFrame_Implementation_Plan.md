...
### Step 6.9: Llama (Ollama) Provider Integration
...
- [ ] `llama_provider_test.ts` - Generates valid plan for a simple prompt (asserts JSON schema compliance)
- [ ] Handles connection errors (Ollama not running)
- [ ] Rejects invalid model names
- [ ] Returns error for invalid JSON output
- [ ] Integration: Plan generated and stored for agent using `codellama:7b-instruct`
- [ ] Provider selection logic routes Llama models to `LlamaProvider`
- [ ] All tests pass, no lint or type errors

**Success Criteria:**

1. [ ] `LlamaProvider` implements `IModelProvider` and passes all tests
2. [ ] Provider factory returns `LlamaProvider` for Llama/Ollama model names
3. [ ] Plans generated by Llama models are valid per `PlanSchema`
4. [ ] All error cases handled and tested
5. [ ] No TypeScript errors, lint warnings, or test failures
